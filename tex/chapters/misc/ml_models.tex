In the following model we will not explicitly state the intercept\footnote{Without loss of generality we can assume that $\bar y = 0$ (otherwise consider the centerd version).} 
...
\subsubsection*{Multivariate Adaptive Regression Splines (MARS)}{
    ...
    To reduce computational complexity, we follow the recommendation from REF\cite{wrapperEarthMultivariateAdaptive2021a} and restrict $h$ in equation~\ref{eq:mars_candidate} to be of degree one (so it is also in a pair of $\mathcal{B}$). Consequently, $\mathcal{C}$ contains functions with a degree of at most 2. 
