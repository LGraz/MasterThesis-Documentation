...
\subsection{Ordinary Least Squares (OLS)}{\label{sec:corr_model_OLS}
    The OLS is a linear model which aims to minimize the sum of the squared residuals. We assume a linear relationship between $y$ and $X$ and allow for Gaussian noise. That is:
    \begin{equation}
        \label{eq:ols}
...
\subsection{Least Absolute Shrinkage and Selection Operator (LASSO)}{\label{sec:corr_model_LASSO}
...
\subsection{General Additive Model (GAM)}{\label{sec:corr_model_GAM}
    ...
    To estimate the non-parametric functions, we can use SS (ref sec.~\ref{sec:Natural_SS}). For this let $\mathcal{S}_j$ be the function which takes some $z\in\R^n$ and returns the SS fitted to $(X_{:,j}, z)$ where the smoothing parameter is optimized by LOOCV\footnote{For efficiency an proxy of the LOOCV is used called generalized cross validation.}.
...
\subsection{Random Forest (RF)}{\label{sec:corr_model_RF}
...
\subsection{Multivariate Adaptive Regression Splines (MARS)}{\label{sec:corr_model_MARS}
