...
\todo[inline]{Ähnliche struktur sich überlegen}
...
\section{Parametric Regression} 
	...
	\subsection{Double Logistic}
		...
		Similar as for the Savitzky-Golay Filter (c.f. section~\ref{sec:Savitzky–Golay}) one can reestimate (only once) the parameters by giving less weight to the overestimated observations and more weight to the underestimated observations. For the details on the choice of the weights we refer to \cite{beckImprovedMonitoringVegetation2006}. We wont apply this reestimation but rather the robustification introduced later in section \ref{sec:loess_robustify}.
	...
	\subsection{Fourier Approximation}
		...
		Analogous to section~\ref{sec:double_logistic} we fit a parametric curve to the data by least squares. Here we take the second order Fourier series:
		$$
			\operatorname{NDVI}(t)=\sum_{j=0}^{2} a_{j} \times \cos \left(j \times \Phi_{t}\right)+b_{j} \times \sin \left(j \times \Phi_{t}  \right)
		$$
		where $\Phi=2 \pi \times(t-1) / n$. Thus, we assume an periodacal behavior. If we would set the period to match one year this would coinced with the nothion that plans grow every year. Example fits can be seen in figure \ref{fig:interpol/fourier_dl_comparison}
...
\section{Non-Parametric Regression}
	...
	\todo{TODO: include Weighted versions}
	...
	\subsection{Kernel Regression}
		...
		$$
			\hat{f}_{X}(x)=\frac{\sum_{i=1}^{n} K\left(\frac{x-x_{i}}{h}\right)}{n h}, \quad \hat{f}_{X, Y}(x, y)=\frac{\sum_{i=1}^{n} K\left(\frac{x-x_{i}}{h}\right) K\left(\frac{y-Y_{i}}{h}\right)}{n h^{2}},
		$$
		where $h$, the bandwidth, symbolizes the windowsize of to consider. By using the above function in equation \eqref{eq:nadaraya} we arrive at the \textit{Nadaraya-Watson} kernel estimator:
	...
	\subsection{Locally Weighted Regression (LOESS)}
		...
		The LOESS introduced by \cite{clevelandRobustLocallyWeighted1979} can be understood as a generalization of the SG Filter (c.f. sec.~\ref{sec:Savitzky-Golay}).
		...
		\subsubsection{Differences between the Robust LOESS and the SG Filter?}
