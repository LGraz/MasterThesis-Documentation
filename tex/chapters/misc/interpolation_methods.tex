...
\section{Non-Parametric Regression}
	...
	\subsection{Kernel Regression: Nadaraya-Watson}
		...
		\end{equation}
		where $h$, the bandwidth, symbolizes the window size of to consider. By using the above function in equation \eqref{eq:nadaraya} we arrive at the Nadaraya-Watson kernel estimator (NW):
	...
	\subsection{Smoothing Splines}
		...
		To define the Smoothing Splines (SS) as in \cite{cravenSmoothingNoisyData1978}, let $\mathcal F$ be the Sobolev space (the space of functions of which the second derivative is integrable). Then the SS are the unique\footnote{Strictly speaking, it is only unique for $\lambda > 0$.} minimizer of
		\begin{equation}
			\label{eq:ss}
