...
\section{Parametric Regression} 
	...
	\subsection{Double Logistic}
		...
		Assumptions:
		\todo[inline]{Die Aufzählung ist hier m.M.n. nicht so passend für einen "Fliesstext".}
		\begin{itemize}
...
\section{Non-Parametric Regression}
	...
	\subsection{Kernel Regression}
		...
		Common choices for the kernel are the normal function or a uniform function (also called `box' function.). Note that we still need to choose the bandwidth\todo{Die Aufzählung ist hier m.M.n. nicht so passend für einen "Fliesstext".} of the function (in the case of the normal function, this is $\sigma$ the standard deviation). For local adaptive bandwidth selection we refer to \cite{brockmannLocallyAdaptiveBandwidth1993}.
	...
	\subsection{Kriging}
		...
		\subsubsection*{Definitions and Assumptions}\todo{ohne unterkapitel struktur}
	...
	\subsection{Locally Weighted Regression (LOESS)}
		...
		The Locally Weighted Regression (LOESS) introduced by \cite{clevelandRobustLocallyWeighted1979} can be understood as a generalization of the SG Filter (c.f. sec.~\ref{sec:Savitzky–Golay}).
