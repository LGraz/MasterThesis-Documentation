...
\section{Goodness of Fit for Selected {{IM}}s}{
	...
	\begin{table}[h]
		\begin{center}
			...
			{{IM}}s (on $P^{SCL45}$) measured with the score functions (that take the LOOCV residuals as input) listed in the left column. $q_X$ denotes here the $X\%$ quantile.}
			\scriptsize
...
\todo[inline]{defiition of RYEA, it is not an accuracy but an error}
	The RYEA for the 28 (in section \ref{sec:corr_itpl_stat}) chosen {{IS}}s is given in table \ref{tab:methods_vs_yieldprediction_relative}. 
	...
	Robustification in the {{IS}}s, does not improve the quality of the fit (measured via the RYEA) in most cases. 
	In addition, SS (rob) are better than DL(rob) in terms of RYEA, with one exception.
	
	The {{IS}} that leads to the lowest RYEA is the OLS-SCL with SS. Given that the OLS-SCL models have very good interpretability, we also present the regression equations below. The corrected NDVI is calculated using 
