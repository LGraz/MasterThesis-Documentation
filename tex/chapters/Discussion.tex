...
\section{Interpolation}{ \label{sec:discussion_itpl}
    ...
    \subsection{Score Functions}{
        In situ data would be best for evaluating various IMs. Being difficult to obtain, \cite{liHighqualityVegetationIndex2021} generated NDVI TS by determining its shape over several years and thereafter added random noise. Because the distribution of the noises is known, the authors also do not have to deal with systematic outliers and can use the RMSE which is not a robust score function. On the other hand, the generated NDVI TS no longer have the typical challenges (underestimated NDVI and data gaps). Thus, the authors test their IMs more for general interpolation reliability and less for challenging S2-derived NDVI TS. Hence, we prefer the LOOCV which keeps those challenges.
        \cite{caiPerformanceSmoothingMethods2017a} evaluate various IMs based solely on the RMSE. Although they also use the `median RMSE', they only calculated the median of different RMSE values from different settings. We see the choice of the RMSE for the selection of the IMs as problematic, because it is sensitive to outliers. Yet, we know that systematic outliers are present in our data (cf. figure~\ref{fig:satelite/time_series_2021_P112/35_scl4_2021-06-03.png}). Thus, when we consider the RMSE as a score function, we do not choose the IM that disregards outliers but is heavily influenced by them. Likewise, \cite{liemohnRMSENotEnough2021} confirms that the RMSE, in the presence of outliers, is not sufficient. \cite{barronGeneralAdaptiveRobust2019} proposes instead a robust score function --- a more flexible variant of the Huber loss function \cite{huberRobustEstimationLocation1964}. However, for simplicity, we choose the robust QAR\textsuperscript{x} score function (cf. definition~\ref{def:qar}). For example, QAR\textsuperscript{90} can easily handle up to 10\% of outliers (i.e., contaminated data points) in the data. 
