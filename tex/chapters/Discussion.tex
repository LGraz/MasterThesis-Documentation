...
\section{Interpolation}{ \label{sec:discussion_itpl}
    \subsection{Data Gaps in Time Series}\label{sec:discussion_itpl_data_gaps}{
        ...
        In our experience, the DL handles data gaps well due to stong shape assumptions (no jumps), but it may happen that the model describes the NDVI increase as abrupt. This, however, was fixed, by bounding the first derivative (cf. section~\ref{sec:itpl_param_optimizationissues}).
    }
    ...
    \subsection{Preselection}{\label{sec:itpl_preselection}
        ...
        We decided against NW because of its systematic errors at peaks and valleys and poor handeling of data gaps (cf. section~\ref{sec:discussion_itpl_data_gaps}). 
    ...
    \subsection{Candidate Selection}{\label{sec:itpl_candiate_selection}
        ...
        In order to not only rely on the form assumptions of the DL, we further choose a non-parametric method for further consideration. Despite the LOESS slightly dominating the SS in table~\ref{tab:cv-statistics_itpl-methods}, we choose the SS. This we justify with the non-smooth behavior of the LOESS in case of data gaps (see section~\ref{sec:discussion_itpl_data_gaps}) and the good interpretability of the SS from minimizing function~\refeq{eq:ss}.
    }

    \subsection{Score Functions}{
        In situ data would be best for evaluating various IMs. Being difficult to obtain, \cite{liHighqualityVegetationIndex2021} generated NDVI TS by determining its shape over several years and thereafter added random noise. Because the distribution of the noises is known, the authors also do not have to deal with systematic outliers and can use the RMSE which is not a robust score funtion. On the other hand, the generated NDVI TS no longer have the typical challenges (unestimated NDVI and data gaps). Thus, the authors test their IMs more for general interpolation reliability and less for challenging S2-derived NDVI TS. Hence, we prefere the LOOCV which keeps those challenges.
        \cite{caiPerformanceSmoothingMethods2017a} evaluate various IMs based solely on the RMSE. Although they also use the `median RMSE', they only calcuateed the median of different RMSE values from different settings. We see the choice of the RMSE for the selection of the IMs as problematic, because it is sensitive to outliers. Yet we know that systematic outliers are present in our data (cf. figure~\ref{fig:satelite/time_series_2021_P112/35_scl4_2021-06-03.png}). Thus, when we consider the RMSE as a score function, we do not choose the IM that disregards outliers but is heavily influenced by them. Likewise, \cite{liemohnRMSENotEnough2021} confirms that the RMSE, in the presence of outliers, is not sufficient. \cite{barronGeneralAdaptiveRobust2019} proposes instead a robust score function --- a more flexible variant of the huber loss function \cite{huberRobustEstimationLocation1964}. However, for simplicity, we choose the robust QAR\textsuperscript{x} score function (cd. definition~\ref{def:qar}). For example, QAR\textsuperscript{90} can easily handle up to 10\% of outliers (i.e., contaminated data points) in the data. 
...
\section{NDVI Correction}{\label{sec:discussion_corr}
    ...
    \subsection{Investigation of Error Sources in Yield Estimation}{\label{sec:discuss_high-rmse-in-yield-prdiction}
        ...
        \end{Nenumerate}
        Even if we would have a perfect NDVI curve, it contains only a fraction of the information about the underlying vegetation. 
