...
\section{Interpolation Setup}{\label{sec:itpl_setup}
	...
	Furthermore, in the subsequent, we denote $w\in \R^n$ as the vector of weights such that $w_i$ corresponds to the weight that $(t_i, y_i)$ should have in the interpolation. Since not all of the following IMs support extrapolation outside of the data range, we will always extrapolate with the closest value within the data range. 
}
...
\section{Robustification}{
	...
	\subsection{Examples and Conclusions}{		
		Examples of the first four iterations using SS and LOESS are shown in figure~\ref{fig:interpol/2x3_SS_robust} and~\ref{fig:interpol/2x3_loess_robust} for six pixels. For the analogous figures of BS and DL we refere to the appendix~\ref{fig:interpol/2x3_B-Splines_robust} and~\ref{fig:interpol/2x3_DL_robust}.
