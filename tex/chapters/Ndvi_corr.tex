\chapter{NDVI Correction XXX.vs.XXX Increase Data Quality} \label{sec:corr}
...
\section{Evaluation via Yield Estimation Accuracy}{
    ...
    In this section, we introduce the relative yield-estimation-accuracy ({RYEA}) and utilize it to evaluate the 28 {{IS}}s from section~\ref{sec:corr_itpl_stat}. 
    % idea
    ...
    Therefore, we want to determine a comparable RYEA for each {{IS}} and choose it as a benchmark criterion. 
    ...
    % \subsubsection*{Yield Estimation}
    {
        ...
        Finally, for each {{IS}}, we calculate the RYEA and describe the results in section \ref{sec:results_ndvi_corr}.
    }
