...
\section{Correction}{
    ...
    {
        We recall the satellite images in figure~\ref{fig:satelite/time_series_2021_P112/35_scl4_2021-06-03.png}, where we had cloudy images despite SCL4 labeled and see fragments in figure~\ref{fig:satelite/time_series_2021_P112/40_scl10_2021-06-28.png} even though we are supposed to see clouds (SCL 10 - Cirrus clouds). The SCL classification is based only on a mixed model trained using the s2 bands.\todo{hier könnte man auch wieder bezug nehmen auf die originale Sektion, wo man SCL einführt. dort fehlt momentan auch die Erklärung, dass SCL ein model output ist}
...
\section{Evaluation Method}{
    ...
    \subsection{Yield Estimation}{
        ...
        \begin{Nitemize}\todo{tabelle wäre sauberer}
            \item Maximum slope
            \item Minimum slope
        ...
        \end{Nitemize}
        For the choice we were inspired by REF-kamir\todo{check reference}. However, we deliberately omit any statistic that involves the minimum (e.g. the NDVI-range), since we regard the minimum as a very error-prone measure due to the large influence of clouds in the time series. 
        
        As a result, for each interpolation strategy, a matrix is obtained in which each row corresponds to a pixel and both the yield and the characterizing statistics\todo{welche characterizing statistics genau?} are contained.
