...
\section{Considering other SCL Classes}{\label{sec:corr_otherSCL}
    ...
    \begin{assumption}{\theequation}%(true NDVI)
        ...
        The `true' NDVI value at time $t$ can be successfully estimated by a (robustified) LOOCV interpolation using high-quality observations. That is, the interpolated value using a (robustified) {{IM}} (cf. chapter~\ref{sec:itpl}) considering the points $P^{SCL45}\setminus P_t$. In the following, we will call this estimate the `true'-NDVI.
    \end{assumption}
