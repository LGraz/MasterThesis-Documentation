...
\section{Evaluation via Yield Estimation Accuracy}{
    ...
    % idea
    The fundamental assumption is that the closer the interpolated NDVI time series is to the true one, the better it can be used to determine crop yield. Implicitly, we believe that an NDVI time series that better models yield will incorporate more true information about the underlying vegetation. 
    ...
    % \subsubsection*{Yield Estimation}
    {
        ...
        We would like to estimate the yield from the NDVI time series produced by all the interpolation strategies for all pixels. However, given the high dimensionality and different lengths of the interpolation (not every time series has the same start and end point), we must first map each NDVI time series into a low-dimensional vector space of covariates. For this, we will use the following statistics:
        {% covaraites
